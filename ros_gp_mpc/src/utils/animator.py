...
class Dynamic3DTrajectory:
    def __init__(
        ...
        self.max_x = (
            self.max_x + 0.25 * (max_range - range_x) + (self.max_x - self.min_x) * 0.2
        )
        self.min_x = (
            self.min_x - 0.25 * (max_range - range_x) - (self.max_x - self.min_x) * 0.2
        )
        self.max_y = (
            self.max_y + 0.25 * (max_range - range_y) + (self.max_y - self.min_y) * 0.2
        )
        self.min_y = (
            self.min_y - 0.25 * (max_range - range_y) - (self.max_y - self.min_y) * 0.2
        )
        self.max_z = (
            self.max_z + 0.25 * (max_range - range_z) + (self.max_z - self.min_z) * 0.2
        )
        self.min_z = (
            self.min_z - 0.25 * (max_range - range_z) - (self.max_z - self.min_z) * 0.2
        )
    ...
    def on_launch(self):
        ...
        self.lines = sum(
            [
                self.ax.plot([], [], [], "-", c=self.colors[i], label=self.legends[i])
                for i in range(self.n_lines)
            ],
            [],
        )
        projection_lines = sum(
            [
                self.ax.plot([], [], [], "-", c=self.colors[i], alpha=0.2)
                for i in range(self.n_lines)
            ],
            [],
        )
        ...
        pos_balls = sum(
            [
                self.ax.plot([], [], [], "o", c=self.colors[i])
                for i in range(self.n_lines)
            ],
            [],
        )
        ...
        vel_bar = [
            self.speed_ax.barh([0], [0], color=self.colors[0], height=bar_height)[0]
        ]
        self.speed_ax.set_xlim(
            [0, np.max(np.sqrt(np.sum(self.vel_data[0] ** 2, -1))) * 1.05]
        )
        ...
        pos_err = []
        p_errors = np.stack(
            [self.data[0] - self.data[i + 1] for i in range(self.n_lines - 1)]
        )

        pos_err += [
            self.pos_err_ax.barh(
                [(i - 1) * bar_height * 1.05],
                [i],
                color=self.colors[i],
                height=bar_height,
        ...
        self.pos_err_ax.set_ylim(
            [-bar_height, ((self.n_lines - 1) * 1.05) * bar_height]
        )
        self.pos_err_ax.set_title("XY position error")
        self.pos_err_ax.set_xlabel(r"$\|\mathbf{p}^* - \mathbf{p}\|\: [m]$")
        self.pos_err_ax.set_yticks(
            [i * bar_height * 1.05 for i in range(self.n_lines - 1)]
        )
        ...
        top_down_lines = sum(
            [
                self.top_down_ax.plot(
                    [], [], color=self.colors[i], label=self.legends[i]
                )
                for i in range(self.n_lines)
            ],
            [],
        ...
        top_down_lines += sum(
            [
                self.top_down_ax.plot([], [], "o", color=self.colors[i])
                for i in range(self.n_lines)
            ],
            [],
        )
        self.top_down_ax.plot(
            self.reference[:, 0], self.reference[:, 1], "-", alpha=0.2
        )
        ...
        x_time_lines = sum(
            [
                self.x_time_ax.plot([], [], color=self.colors[i], label=self.legends[i])
                for i in range(1, self.n_lines)
            ],
            [],
        )
        ...
        y_time_lines = sum(
            [
                self.y_time_ax.plot([], [], color=self.colors[i], label=self.legends[i])
                for i in range(1, self.n_lines)
            ],
            [],
        )
    ...
    def animate(self, i):
        ...
        for j, (line, xi) in enumerate(
            zip(self.lines[: self.n_lines], self.sparsed_data)
        ):
            x, y, z = xi[:i].T
            ...
            if len(x) > 0:
                ...
                self.lines[self.top_down_0_idx + j + self.n_lines].set_data(
                    x[-1], y[-1]
                )

            self.lines[self.top_down_0_idx + j].set_data(x, y)

        self.lines[self.vel_bars_0_idx].set_width(
            np.sqrt(np.sum(self.vel_data[0, i, :] ** 2))
        )
        ...
        for j in reversed(range(self.n_lines - 1)):
            self.lines[self.x_time_0_idx + j].set_data(
                self.t_vec[:i], np.abs(self.data[0, :i, 0] - self.data[j + 1, :i, 0])
            )
            self.lines[self.y_time_0_idx + j].set_data(
                self.t_vec[:i], np.abs(self.data[0, :i, 1] - self.data[j + 1, :i, 1])
            )

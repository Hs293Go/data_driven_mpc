...
def apply_map_limits(x, y, z, limits):
    ...
    if (
        x_actual_range > x_max_range
        or y_actual_range > y_max_range
        or z_actual_range > z_max_range
    ):
        shrink_ratio = max(
            x_actual_range / x_max_range,
            y_actual_range / y_max_range,
            z_actual_range / z_max_range,
        )
...
def random_periodical_trajectory(plot=False, random_state=None, map_limits=None):
    ...
    kernel_y = ExpSineSquared(length_scale=4.5, periodicity=30) + ExpSineSquared(
        length_scale=4.0, periodicity=15
    )
    kernel_x = ExpSineSquared(length_scale=4.5, periodicity=30) + ExpSineSquared(
        length_scale=4.5, periodicity=60
    )
    ...
    if map_limits is not None:
        x_sample_hr, y_sample_hr, z_sample_hr = apply_map_limits(
            x_sample_hr, y_sample_hr, z_sample_hr, map_limits
        )
    ...
    if plot:
        ...
        shortest_axis = min(
            np.max(x_sample_hr) - np.min(x_sample_hr),

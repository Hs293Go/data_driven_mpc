...
def draw_arrow(x_base, y_base, x_body, y_body):
    ...
    lower_arrow = beta_rot.dot(
        np.array([[-np.cos(np.pi / 6)], [-np.sin(np.pi / 6)]]) * len_arrow / 3
    )
    upper_arrow = beta_rot.dot(
        np.array([[-np.cos(np.pi / 6)], [np.sin(np.pi / 6)]]) * len_arrow / 3
    )
    ...
    return (
        [
            x_base,
            x_base + x_body,
            x_base + x_body + lower_arrow[0, 0],
            x_base + x_body,
            x_base + x_body + upper_arrow[0, 0],
        ],
        [
            y_base,
            y_base + y_body,
            y_base + y_body + lower_arrow[1, 0],
            y_base + y_body,
            y_base + y_body + upper_arrow[1, 0],
        ],
    )
...
def visualize_data_distribution(x_data, y_data, clusters, x_pruned, y_pruned):
    ...
    for i in range(int(n_clusters)):
        ax.scatter(
            x_pruned[clusters[i], 0],
            x_pruned[clusters[i], 1],
            x_pruned[clusters[i], 2],
            alpha=0.6,
        )
...
def visualize_gp_inference(
    x_data, u_data, y_data, gp_ensemble, vis_features_x, y_dims, labels
):
    ...
    if len(vis_features_x) == 1:
        ...
        im = ax[0].scatter(
            x_data[:, vis_features_x[0]],
            x_data[:, vis_features_x[1]],
            x_data[:, vis_features_x[2]],
            c=y_mse,
            cmap="viridis",
            alpha=0.6,
            vmin=v_min,
        ...
        ax[1].set_title(
            r"GP prediction mangnitude $\|\tilde{\mathbf{a}}^e\|$", size=font_size
        )
        ...
        cbar.set_ylabel(
            r"$\|\mathbf{a}^e\|\left[\frac{m}{s^2}\right]$",
            size=font_size,
            labelpad=20,
            rotation=270,
        )
    ...
    x = np.linspace(
        min(x_data[:, vis_features_x[0]]), max(x_data[:, vis_features_x[0]]), 100
    )
    y = np.linspace(
        min(x_data[:, vis_features_x[1]]), max(x_data[:, vis_features_x[1]]), 100
    )
    ...
    def init():
        ...
        ax.set_zlabel(
            r"$\|\tilde{\mathbf{a}}^e\|\: \left[\frac{m}{s^2}\right]$",
            size=16,
            labelpad=10,
        )
        ax.set_title(
            r"GP correction. Slice $v_z=0 \:\: \left[\frac{m}{s}\right]$", size=18
        )
        return (fig,)

    def animate(i):
        ax.view_init(elev=30.0, azim=i * 3)
        return fig

    _ = animation.FuncAnimation(
        fig, animate, init_func=init, frames=360, interval=20, blit=False
    )
...
def initialize_drone_plotter(world_rad, quad_rad, n_props, full_traj=None):
    ...
    artists = {
        ...
        "missing_targets": ax.plot(
            [], [], [], color="r", marker="o", linestyle="None", markersize=12
        )[0],
        "reached_targets": ax.plot(
            [], [], [], color="g", marker="o", linestyle="None", markersize=12
        )[0],
        "sim_trajectory": [
            ax.plot([], [], [], "-", color="tab:blue", alpha=0.9 - i * 0.2 / n_props)[0]
            for i in range(n_props)
        ],
        "int_trajectory": [
            ax.plot([], [], [], "-", color="tab:orange", alpha=0.9 - i * 0.5 / n_props)[
                0
            ]
            for i in range(n_props + 1)
        ],
        "prop_trajectory": [
            ax.plot([], [], [], "-", color="tab:red", alpha=0.9 - i * 0.2 / n_props)[0]
            for i in range(n_props)
        ],
        "prop_covariance": [
            ax.plot([], [], [], color="r", alpha=0.5 - i * 0.45 / n_props)[0]
            for i in range(n_props)
        ],
        "projection_lines": [
...
def draw_drone_simulation(
    ...
    trajectories_artist = (
        artists["trajectory"] if "trajectory" in artists.keys() else []
    )
    projected_traj_artists = (
        artists["projection_lines"] if "projection_lines" in artists.keys() else []
    )
    drone_sketch_artist = artists["drone"] if "drone" in artists.keys() else []
    drone_sketch_artist_x_motor = (
        artists["drone_x"] if "drone_x" in artists.keys() else []
    )
    targets_artist = (
        artists["missing_targets"] if "missing_targets" in artists.keys() else []
    )
    reached_targets_artist = (
        artists["reached_targets"] if "reached_targets" in artists.keys() else []
    )
    projected_tar_artists = (
        artists["projection_target"] if "projection_target" in artists.keys() else []
    )
    sim_traj_artists = (
        artists["sim_trajectory"] if "sim_trajectory" in artists.keys() else []
    )
    int_traj_artists = (
        artists["int_trajectory"] if "int_trajectory" in artists.keys() else []
    )
    pred_traj_artists = (
        artists["prop_trajectory"] if "prop_trajectory" in artists.keys() else []
    )
    cov_artists = (
        artists["prop_covariance"] if "prop_covariance" in artists.keys() else []
    )
    ...
    def draw_fading_traj(traj, traj_artists):
        ...
        for j in range(min(traj.shape[0] - 1, len(traj_artists))):
            traj_artists[j].set_data(
                np.array([traj[j, 0], traj[j + 1, 0]]),
                np.array([traj[j, 1], traj[j + 1, 1]]),
            )
    ...
    if targets is not None and targets_reached is not None:
        ...
        if missing.any():
            ...
            [
                ax.draw_artist(projected_tar_artist)
                for projected_tar_artist in projected_tar_artists
            ]

    # Draw quadrotor trajectory
    trajectory_start_pt = max(len(x_trajectory) - 100, 0)
    trajectories_artist.set_data(
        x_trajectory[trajectory_start_pt:, 0], x_trajectory[trajectory_start_pt:, 1]
    )
    ...
    projected_traj_artists[0].set_data(
        x_trajectory[trajectory_start_pt:, 0], ax.get_ylim()[1]
    )
    projected_traj_artists[0].set_3d_properties(x_trajectory[trajectory_start_pt:, 2])
    projected_traj_artists[1].set_data(
        np.atleast_1d([ax.get_xlim()[0]] * (len(x_trajectory) - trajectory_start_pt)),
    ...
    # Draw drone art
    drone_art = draw_drone(
        x_trajectory[-1, 0:3], x_trajectory[-1, 3:7], quad.x_f, quad.y_f
    )
...
def trajectory_tracking_results(
    ...
    q_euler = np.stack(
        [quaternion_to_euler(x_executed[j, 3:7]) for j in range(x_executed.shape[0])]
    )
    for i in range(3):
        ax[i, 1].plot(t_ref, q_euler[:, i], label=legend_labels[1])
    if with_ref:
        ref_euler = np.stack(
            [quaternion_to_euler(x_ref[j, 3:7]) for j in range(x_ref.shape[0])]
        )
        q_err = []
...
def mse_tracking_experiment_plot(
    ...
    for seed_id, track_seed in enumerate(traj_type_vec):
        for j in range(variants_dim):
            for i, _ in enumerate(train_samples_vec):
                mse_data = (
                    mse[seed_id, :, i, j] if len(mse.shape) == 4 else mse[seed_id, :, i]
                )
                label = legends[i] if seed_id == 0 and j == 0 else None
                if legends[i] == "perfect":
                    axes[j, seed_id].plot(
                        v_max[seed_id, :], mse_data, "--o", linewidth=4, label=label
                    )
                else:
                    axes[j, seed_id].plot(
                        v_max[seed_id, :], mse_data, "--o", label=label
                    )
            if seed_id == 0:
                axes[j, seed_id].set_ylabel(y_labels[j], size=font_size)
            if j == 0:
                axes[j, seed_id].set_title(
                    "RMSE [m] | " + str(track_seed), size=font_size + 2
                )
    ...
    for i, n_train in enumerate(train_samples_vec):
        plt.plot(
            v[ind_v],
            t_opt.reshape(t_opt.shape[0] * t_opt.shape[1], -1)[ind_v, i],
            label=legends[i],
        )
    fig.legend(
        loc="upper center",
...
def get_experiment_files():
    ...
    if not os.path.exists(metadata_file):
        safe_mknode_recursive(
            os.path.join(results_path, "experiments"), "metadata.json", overwrite=False
        )
    if not os.path.exists(mse_file):
        safe_mknode_recursive(
            os.path.join(results_path, "experiments"), "mse.npy", overwrite=False
        )
    if not os.path.exists(mean_v_file):
        safe_mknode_recursive(
            os.path.join(results_path, "experiments"), "mean_v.npy", overwrite=False
        )
    if not os.path.exists(t_opt_file):
        safe_mknode_recursive(
            os.path.join(results_path, "experiments"), "t_opt.npy", overwrite=False
        )

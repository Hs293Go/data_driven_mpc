...
def gp_visualization_experiment(
    quad_sim_options,
    dataset_name,
    x_cap,
    hist_bins,
    ...
    labels_x = [
        ...
        r"$p_y\:\left[m\right]$",
        r"$p_z\:\left[m\right]$",
        r"$q_w\:\left[rad\right]$",
        r"$q_x\:\left[rad\right]$",
        r"$q_y\:\left[rad\right]$",
        r"$q_z\:\left[rad\right]$",
        r"$v_x\:\left[\frac{m}{s}\right]$",
        r"$v_y\:\left[\frac{m}{s}\right]$",
        r"$v_z\:\left[\frac{m}{s}\right]$",
        r"$w_x\:\left[\frac{rad}{s}\right]$",
        r"$w_y\:\left[\frac{rad}{s}\right]$",
        r"$w_z\:\left[\frac{rad}{s}\right]$",
    ]
    ...
    if isinstance(dataset_name, str):
        ...
        test_gp_ds = GPDataset(
            test_ds,
            x_features=x_vis_feats,
            u_features=u_vis_feats,
            y_dim=y_vis_feats,
            cap=x_cap,
            n_bins=hist_bins,
            thresh=hist_thresh,
    ...
    if grid_sampling_viz:
        visualize_gp_inference(
            x_test, u_test, y_test, gp_ensemble, x_vis_feats, y_dims, labels
        )
    ...
    for i in range(std_estimate.shape[1]):
        ...
        if nominal_diff is not None:
            ...
            plt.title(
                "Mean dt: %.2f s. Augmented RMSE: %.5f [m/s]"

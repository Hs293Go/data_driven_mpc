...
from src.experiments.point_tracking_and_record import (
    check_out_data,
    get_record_file_and_dir,
    make_record_dict,
)
...
def odometry_skipped_warning(last_seq, current_seq, stage):
    skip_msg = "Odometry skipped at %s step. Last: %d, current: %d" % (
        stage,
        last_seq,
        current_seq,
    )
...
class GPMPCWrapper:
    def __init__(
        ...
        if environment == "gazebo":
            try:
                get_gazebo_physics = rospy.ServiceProxy(
        ...
        self.control_freq_factor = rospy.get_param(
            "~control_freq_factor", default=5 if environment == "gazebo" else 6
        )
        ...
        if load_options is not None:
            rospy.loginfo(
                "Attempting to load GP model from:\n   git: {}\n   name: {}\n   meta: {}".format(
                    load_options["git"],
                    load_options["model_name"],
                    load_options["params"],
                )
            )
            ...
            if pre_trained_models is None:
                rospy.logwarn(
                    "Model parameters specified did not match with any pre-trained GP"
                )
        else:
        ...
        self.gp_mpc = ROSGPMPC(
            self.t_horizon,
            self.n_mpc_nodes,
            self.opt_dt,
            quad_name=quad_name,
            point_reference=False,
            gp_models=pre_trained_models,
            rdrv=rdrv,
        )
        ...
        if recording_options["recording"]:
            ...
            rec_dict, rec_file = get_record_file_and_dir(
                ...
                recording_options,
                simulation_setup=metadata,
                overwrite=overwrite,
            )

            # If flying with the optitrack system, also record raw optitrack estimates
            if (
                self.environment == "flying_room"
                or self.environment == "arena"
                and record_raw_optitrack
            ):
        ...
        self.control_pub = rospy.Publisher(
            control_topic, ControlCommand, queue_size=1, tcp_nodelay=True
        )
        ...
        self.ref_sub = rospy.Subscriber(
            reference_topic, ReferenceTrajectory, self.reference_callback
        )
        if ekf_odom_topic:
        ...
        if raw_topic is not None and record_raw_optitrack:
            self.raw_sub = rospy.Subscriber(
                raw_topic, PoseStamped, self.raw_odometry_callback
            )
    ...
    def rest_state(self):
        ...
        self.last_u_ref = (
            self.gp_mpc.quad.g[-1]
            * self.gp_mpc.quad.mass
            / (self.gp_mpc.quad.max_thrust * 4)
        )
    ...
    def run_mpc(self, odom, recording=True):
        ...
        if w_opt is not None:
            ...
            if not self.recording_warmup and recording and self.x_initial_reached:
                ...
                self.rec_dict = check_out_data(
                    self.rec_dict, x_out, None, self.w_opt, dt
                )

            self.w_opt = w_opt
            if (
                self.x_initial_reached
                and self.current_idx < self.quad_controls.shape[0]
            ):
                self.quad_controls[self.current_idx, :] = np.expand_dims(
                    self.w_opt[:4], axis=0
                )
    ...
    def check_out_initial_state(self, odom):
        ...
        if self.w_opt is not None:
            ...
            self.rec_dict["state_in"] = np.append(
                self.rec_dict["state_in"], np.array(self.x)[np.newaxis, :], 0
            )
            self.rec_dict["timestamp"] = np.append(
                self.rec_dict["timestamp"], odom.header.stamp.to_time()
            )
            if self.current_idx < self.x_ref.shape[0]:
                self.rec_dict["state_ref"] = np.append(
                    self.rec_dict["state_ref"],
                    self.x_ref[np.newaxis, self.current_idx, :],
                    0,
                )
    ...
    def odometry_callback(self, msg):
        ...
        # Run MPC
        if (
            msg.header.seq > self.last_odom_seq_number + 2
            and self.last_odom_seq_number > 0
            and self.x_initial_reached
        ):
            ...
            self.optimize_next = True
            self.last_odom_seq_number = msg.header.seq
            odometry_skipped_warning(
                self.last_odom_seq_number, msg.header.seq, "optimization"
            )
            return
    ...
    def raw_odometry_callback(self, msg):
        ...
        self.rec_dict_raw["state_in"] = np.append(
            self.rec_dict_raw["state_in"], x[np.newaxis, :], 0
        )
        self.rec_dict_raw["timestamp"] = np.append(
            self.rec_dict_raw["timestamp"], msg.header.stamp.to_time()
        )
    ...
    def set_reference(self):
        ...
        if self.landing:
            ...
            x_ref[0][2] = (
                min(0.1, self.x[2] + dz) if dz > 0 else max(0.1, self.x[2] + dz)
            )
            ...
            if abs(self.x[2] - 0.1) < 0.05:
                ...
                n_vel = len(
                    self.metadata_dict[self.ref_traj_name][self.model_name].keys()
                )
                ...
                for traj_id, traj_type in enumerate(self.metadata_dict.keys()):
                    for model_id, model_type in enumerate(
                        self.metadata_dict[traj_type].keys()
                    ):
                        for vel_id, vel in enumerate(
                            self.metadata_dict[traj_type][model_type].keys()
                        ):
                            self.mse_exp[
                                traj_id, vel_id, model_id, 0
                            ] = self.metadata_dict[traj_type][model_type][vel][0]
                ...
                rospy.loginfo(
                    "Tracking complete. Total RMSE: %.5f m. Max axial vel: %.3f. "
                    "Mean optimization time: %.3f ms"
                    % (rmse, v_max, self.optimization_dt * 1000)
                )

                self.current_idx = 0
                if self.plot:
                    with_gp = (
                        " + GP " if self.pre_trained_models is not None else " - GP "
                    )
                    tit = r"$v_{max}$=%.2f m/s | RMSE: %.4f | %s " % (
                        v_max,
                        float(rmse),
                        with_gp,
                    )
        ...
        if self.x_ref is None:
            ...
            if self.x_ref_prov is None:
                rospy.loginfo(
                    "Entering provisional hovering mode while to reference is available at: "
                )
        ...
        if (
            quaternion_state_mse(np.array(self.x), self.x_ref[0, :], mask) < th
            and not self.x_initial_reached
        ):
            ...
            model_data = self.gp_mpc.set_reference(
                separate_variables(self.x_ref[:1, :]), self.u_ref[:1, :]
            )
        ...
        if not self.x_initial_reached:
            ...
            x_ref[0][0] = (
                min(self.x_ref[0, 0], self.x[0] + dx)
                if dx > 0
                else max(self.x_ref[0, 0], self.x[0] + dx)
            )
            x_ref[0][1] = (
                min(self.x_ref[0, 1], self.x[1] + dy)
                if dy > 0
                else max(self.x_ref[0, 1], self.x[1] + dy)
            )
            x_ref[0][2] = (
                min(self.x_ref[0, 2], self.x[2] + dz)
                if dz > 0
                else max(self.x_ref[0, 2], self.x[2] + dz)
            )
            ...
            ref_traj = self.x_ref[
                ...
                + self.n_mpc_nodes * self.control_freq_factor,
                :,
            ]
            ref_u = self.u_ref[
                self.current_idx : self.current_idx
            ...
            downsample_ref_ind = np.arange(
                0,
                min(self.control_freq_factor * self.n_mpc_nodes, ref_traj.shape[0]),
                self.control_freq_factor,
                dtype=int,
            )
    ...
    def plot_tracking_mse_experiment(self):
        ...
        mse_tracking_experiment_plot(
            v_max=self.mse_exp_v_max,
            mse=self.mse_exp,
            traj_type_vec=traj_type_labels,
            train_samples_vec=model_type_labels,
            legends=model_type_labels,
            y_labels=["RotorS"],
            t_opt=self.t_opt,
        )
    ...
    def save_recording_data(self):
        ...
        for i in tqdm(range(len(self.rec_dict["input_in"]))):
            ...
            x_pred, _ = self.gp_mpc.quad_mpc.forward_prop(
                x_0, u, t_horizon=dt, use_gp=False
            )
            x_pred = x_pred[-1, np.newaxis, :]

            self.rec_dict["state_pred"] = np.append(
                self.rec_dict["state_pred"], x_pred, axis=0
            )
            self.rec_dict["error"] = np.append(
                self.rec_dict["error"], x_f - x_pred, axis=0
            )
        ...
        if self.rec_dict_raw is not None:
            data_len = min(
                self.rec_dict_raw["state_in"].shape[0],
                len(self.rec_dict_raw["timestamp"]),
            )
...
def main():
    ...
    load_options = {"git": "b6e73a5", "model_name": "", "params": None}
    ...
    plot = True
    plot = (
        rospy.get_param("~plot", default=None)
        if rospy.get_param("~plot", default=None) is not None
        else plot
    )

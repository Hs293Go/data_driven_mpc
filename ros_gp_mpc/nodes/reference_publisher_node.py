...
class ReferenceGenerator:
    def __init__(self):
        ...
        control_freq_factor = rospy.get_param(
            "~control_freq_factor", default=5 if quad_name == "hummingbird" else 6
        )
        opt_dt = t_horizon / (n_mpc_nodes * control_freq_factor)

        reference_topic = "reference"
        status_topic = "busy"
        reference_pub = rospy.Publisher(
            reference_topic, ReferenceTrajectory, queue_size=1
        )
        ...
        while not rospy.is_shutdown():
            ...
            if not self.gp_mpc_busy and mode == "loop":
                ...
                x_ref, t_ref, u_ref = loop_trajectory(
                    quad,
                    opt_dt,
                    v_max=loop_v_max,
                    radius=loop_r,
                ...
                x_ref, t_ref, u_ref = lemniscate_trajectory(
                    quad,
                    opt_dt,
                    v_max=loop_v_max,
                    radius=loop_r,
                ...
                speed = v_list[v_ind]
                log_msg = (
                    "Random trajectory generator %d/%d. Seed: %d. Mean vel: %.3f m/s"
                    % (curr_trajectory_ind + 1, n_trajectories, seed, speed)
                )
                rospy.loginfo(log_msg)

                x_ref, t_ref, u_ref = random_trajectory(
                    quad, opt_dt, seed=seed, speed=speed, map_name=map_limits, plot=plot
                )
                msg = ReferenceTrajectory()
